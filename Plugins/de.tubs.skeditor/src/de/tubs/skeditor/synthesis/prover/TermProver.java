...
import de.tubs.skeditor.contracting.grammar.folParser.FunctioncallContext;
...
public class TermProver {
	...
	class ExpressionVisitor extends folBaseVisitor<Expr> {
		...
		public Expr visitCompareformula(folParser.CompareformulaContext ctx) {
			...
			/*
			 * ArithExpr first = (ArithExpr) visitSummformula(ctx.summformula(0)); for(int i
			 * = 1; i < ctx.summformula().size(); i++) { ArithExpr expr = (ArithExpr)
			 * visitSummformula(ctx.summformula(i)); switch(ctx.compoperator(i-1).getText())
			 * { case ">": result = getContext().mkGt(first, expr); first = expr; break;
			 * case "<": result = getContext().mkLt(first, expr); first = expr; break; case
			 * ">=": result = getContext().mkGe(first, expr); first = expr; break; case
			 * "<=": result = getContext().mkLe(first, expr); first = expr; break; case
			 * "==": result = getContext().mkEq(first, expr); first = expr; break; case
			 * "!=": result = getContext().mkNot(getContext().mkEq(first, expr)); first =
			 * expr; break; default: return null; }
			 * 
			 * }
			 */
			return result;
